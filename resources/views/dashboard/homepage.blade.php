...
@section('content')
    ...
    <div class="container" style="max-width: 1500px;margin-top: 30px;margin-bottom: 50px">
        ...
        @endif
        <h3 style="letter-spacing: 3px;margin-top: 20px" class="mt-4 mb-3">Home Page</h3>
        {{--            <div>--}}
        {{--                <button data-toggle="modal" data-target="#myModal" class="btn btn-success">ADD NEW OFFICE</button>--}}
        {{--            </div>--}}

        <div>
            <form action="{{url('update-home')}}" method="post" enctype="multipart/form-data">
                @csrf
                <p style="font-weight: bold">Update Main Text</p>
                <textarea name="main_text" required>{{$data['main_text']}}</textarea><br>
                <p style="font-weight: bold">Update Banner Image</p>
